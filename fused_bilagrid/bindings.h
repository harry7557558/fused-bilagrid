...
torch::Tensor bilagrid_normal_uniform_sample_forward_tensor(
    torch::Tensor bilagrid, // [N,3,L,H,W]
    torch::Tensor rgb  // [N,m,h,w,3]
) {
    int N = bilagrid.size(0), L = bilagrid.size(2),
        H = bilagrid.size(3), W = bilagrid.size(4);
...
std::tuple<torch::Tensor, torch::Tensor>
bilagrid_normal_uniform_sample_backward_tensor(
    torch::Tensor bilagrid,  // [N,3,L,H,W]
    torch::Tensor rgb,  // [N,m,h,w,3]
    torch::Tensor v_output,  // [N,m,h,w,3]
    const int version,
    ...
    auto opts = rgb.options();
    auto v_bilagrid = torch::zeros({N,3,L,H,W}, opts);
    auto v_rgb = torch::empty({N,m,h,w,3}, opts);
...
torch::Tensor bilagrid_normal_patched_sample_forward_tensor(
    torch::Tensor bilagrid, // [N,3,L,H,W]
    torch::Tensor rgb,  // [N,m,h,w,3]
    int h0, int w0,
    torch::Tensor offsets  // [N,m,2]
...
std::tuple<torch::Tensor, torch::Tensor>
bilagrid_normal_patched_sample_backward_tensor(
    torch::Tensor bilagrid,  // [N,3,L,H,W]
    torch::Tensor rgb,  // [N,m,h,w,3]
    int h0, int w0,
    torch::Tensor offsets,  // [N,m,2]
    ...
    // bilagrid_normal_patched_sample_backward_v2(
    bilagrid_normal_patched_sample_backward_v1(  // TODO: v1 is extremely slow for patched sample
        bilagrid.data_ptr<float>(),
        rgb.data_ptr<float>(),
        offsets.data_ptr<int>(),
        v_output.data_ptr<float>(),
