...
__global__ void find_row_max_kernel(
    ...
    for (int i = start + threadIdx.x; i < end; i += blockDim.x) {
        uint32_t u = __float_as_uint(row_ptr[i]);
...
__global__ void select_pass_kernel(
    ...
    if (pass == 0) {
        ...
        const double qq = std::min(1.0, std::max(0.0, static_cast<double>(q)));
        const uint32_t rank = static_cast<uint32_t>(qq * static_cast<double>(masked_count - 1u));
