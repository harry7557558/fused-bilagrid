...
class _FusedUniformGridPPISPSample(torch.autograd.Function):
    @staticmethod
    def forward(ctx, bilagrid, rgb, _args):
        with torch.cuda.device(bilagrid.device):
            output = _C.bilagrid_ppisp_uniform_sample_forward(bilagrid, rgb)
        ctx.save_for_backward(bilagrid, rgb)
        ctx._args = _args
        return output
    ...
    def backward(ctx, v_output):
        bilagrid, rgb = ctx.saved_tensors
        with torch.cuda.device(bilagrid.device):
            return *_C.bilagrid_ppisp_uniform_sample_backward(
                bilagrid, rgb, v_output.contiguous(),
                *ctx._args
            ), None
...
class _FusedPatchedGridPPISPSample(torch.autograd.Function):
    @staticmethod
    def forward(ctx, bilagrid, rgb, h0, w0, offsets):
        with torch.cuda.device(bilagrid.device):
            output = _C.bilagrid_ppisp_patched_sample_forward(bilagrid, rgb, h0, w0, offsets)
        ctx.save_for_backward(bilagrid, rgb, offsets)
        ctx.h0, ctx.w0 = h0, w0
        return output
...
def fused_bilagrid_depth_sample(
        bilagrid, coords, depth, compute_coords_grad=False,
        actual_height=None, actual_width=None, patch_offsets=None,
        image_indices=None
    ):
    ...
    if image_indices is not None:
        raise NotImplementedError()
        return _FusedPackedGridDepthSample.apply(
            bilagrid.float().contiguous(),
            image_indices.contiguous(),
            coords.float().contiguous(),
            rgb.float().contiguous(),
    ...
    if coords is not None:
        raise NotImplementedError()
        return _FusedGridDepthSample.apply(
            bilagrid.float().contiguous(),
            coords.float().contiguous(),
            rgb.float().contiguous(),
            compute_coords_grad
        ...
        # TODO: re-calibrate
        args = choose_uniform_sample_backward_args(*map(int, depth.shape[-3:-1]), *map(int, bilagrid.shape[-3:]))
        # args = (1, 8, 8, 5)
...
class BilateralGrid(nn.Module):
    ...
    def __init__(self, num, grid_X=16, grid_Y=16, grid_W=8):
        ...
        self.fused_bilagrid_sample = fused_bilagrid_sample
    ...
    def forward(
        ...
        rgb = self.fused_bilagrid_sample(
...
class BilateralGridPPISP(BilateralGrid):
    ...
    def __init__(self, num, grid_X=16, grid_Y=16, grid_W=8):
        ...
        Args:
            ...
            grid_W (int): Defines grid guidance dimension $L$.
        """
        super(BilateralGridPPISP, self).__init__(num, grid_X, grid_Y, grid_W)
...
class BilateralGridDepth(BilateralGrid):
    ...
    def __init__(self, num, grid_X=16, grid_Y=16, grid_W=8):
        ...
        Args:
            ...
            grid_W (int): Defines grid guidance dimension $L$.
        """
        super(BilateralGridDepth, self).__init__(num, grid_X, grid_Y, grid_W)
